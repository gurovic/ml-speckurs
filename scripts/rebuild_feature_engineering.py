#!/usr/bin/env python3
"""Rebuild lesson 14_feature_engineering: theory, exercises, practice, solution.
...
FOLDER = Path(__file__).resolve().parents[1] / "14_feature_engineering"
...
**Сквозной пример.** Все разделы работают с одной таблицей: объявления о продаже квартир, цель — предсказать **цену** (млн руб.). Один объект — одно объявление. Сразу делим на train и validation (70 % / 30 %), как на занятии 25: всё, что «обучается» по данным, будем считать только на train."""
        ),
        code(FLATS_CODE),
        md(
...
Нарушение пп. 1–2 называется **утечкой** (занятие 25): модель получает информацию, которой в реальности не будет, и validation-оценка становится обманчиво хорошей."""
        ),
        md(
...
Линейная модель (занятие 29) складывает вклады признаков по отдельности. Новые столбцы позволяют ей описывать более сложные связи:
...
5. test не трогать — он для финала (занятие 33).

Проверим гипотезу из п. 2: помогает ли `площадь_на_комнату` предсказывать цену. Модель — знакомый kNN (занятие 25), метрика — MAE (средняя абсолютная ошибка, млн руб.).
...
def practice_cells(with_solutions: bool) -> list[dict]:
    ...
    header = (
        """# Занятие 28. Практика: признаки для цены квартиры (~90 мин)

Вы **пишете весь код сами**. Ячейку **«Дано»** не меняйте.

Задача — регрессия: предсказать **цену** квартиры (млн руб.) по объявлению. Модель для проверки идей — kNN-регрессия (занятие 25), метрика — **MAE**. Теория — `feature_engineering_theory.ipynb` (занятие 27).
