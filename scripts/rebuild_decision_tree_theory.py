...
На занятии 29 вы уже видели, что сложная модель без ограничений переобучается. Дерево — яркий пример: оно легко строит нелинейные правила и понятно человеку, но глубокое дерево запоминает шум.
...
В **классификации** (пп. 3–4) дерево уменьшает Gini. В **регрессии** (`DecisionTreeRegressor`) критерий другой — **MSE** (средний квадрат ошибки, занятие 25):
...
**Пропуски (`NaN`).** В новых версиях sklearn дерево может направлять объекты с пропуском влево или вправо. Либо заполнение пропусков до обучения (занятие 23) — зафиксируйте стратегию в проекте.

**Категории.** `DecisionTreeClassifier` не принимает строки напрямую — нужен `OneHotEncoder` или ordinal encoding (занятие 23).
