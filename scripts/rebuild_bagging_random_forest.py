...
На занятии 31 одно дерево было **понятным**, но **нестабильным**: маленькое изменение train меняло верхние вопросы. Логичный вопрос: а что, если взять **много** деревьев и объединить их прогнозы?
...
**Одна сильная гибкая модель** часто даёт **малый bias**, но **большой variance** (занятие 29). **Ансамбль** обычно **снижает variance**, почти не увеличивая bias — отсюда выигрыш над одним деревом."""
        ),
        md(
...
| **Boosting** | Последовательно, каждая модель исправляет ошибки предыдущих | **Смещение** (bias) | `GradientBoostingClassifier` (занятие 36) |
...
На занятии 31 глубокое дерево легко **переобучается**. Ещё одна проблема — **нестабильность**: два дерева на слегка разных bootstrap-выборках часто дают **разные** верхние вопросы.
