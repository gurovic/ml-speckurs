...
def clean_raw(df):
    df = df.copy()
    df["district"] = df["district"].astype(str).str.replace(r"^[A-Z]+", "", regex=True)
    distance = df["distance_to_center_km"].astype(str).str.replace("O", "0", regex=False)
    df["distance_to_center_km"] = pd.to_numeric(distance, errors="coerce")
    return df.drop(columns=["metro_h"])


train = clean_raw(train)
test = clean_raw(test)
"""
    ),
    code(
