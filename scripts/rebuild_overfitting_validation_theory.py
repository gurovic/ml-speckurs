...
NOTEBOOK = (
    ...
    / "17_overfitting_validation/overfitting_validation_theory.ipynb"
)
...
Ошибку измеряем знакомой метрикой **MSE** (занятие 29, пп. 4 и 6): чем меньше, тем лучше."""
        ),
        md(
...
- есть **утечка данных** — validation «подсмотрела» в обучение (определение — занятие 25, п. 10; варианты для CV — пп. 12–13 этого занятия).

Регуляризация (занятие 29, п. 13) и ограничение сложности помогают, но **не заменяют** честную проверку на отложенных данных."""
        ),
        md(
...
Роли трёх выборок вы уже знаете (занятие 25, п. 4): train — обучение параметров (`fit`), validation — сравнение идей и подбор настроек, test — финальная проверка **один раз**.
...
На практике (занятие 34) для validation curve достаточно **train + validation**; test вводим как принцип финальной проверки (п. 15)."""
        ),
        code(
            """import numpy as np
