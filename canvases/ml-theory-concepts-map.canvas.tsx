...
const LABEL_SPLIT: Record<string, string[]> = {
  ...
  "Дисбаланс классов в random forest": [
...
const LABEL_RENAME: Record<string, string> = {
  "Feature importance в дереве": "Impurity importance в дереве",
...
const LESSONS: Lesson[] = [
  ...
  },
  {
    ...
    short: "признаки (FE)",
    concepts: [
      { label: "Критерии хорошего признака" },
      { label: "Числовые преобразования (отношения, log1p)" },
      { label: "Интервалы и ограничение выбросов" },
      { label: "Нелинейность через новые признаки (x², x₁x₂)" },
      { label: "Когда нужно взаимодействие признаков" },
      { label: "Даты, сезонность и цикличность (sin/cos)" },
      { label: "Время с прошлого события (recency)" },
      { label: "Категориальные признаки и one-hot" },
      { label: "Порядковые категории (ordinal encoding)" },
      { label: "Редкие и неизвестные категории в test" },
      { label: "Пропуски — заполнение и индикатор" },
      { label: "Почему появляются пропуски (MNAR и др.)" },
      { label: "Агрегаты по прошлым событиям" },
      { label: "Утечка через агрегаты и target encoding" },
      { label: "Масштабирование StandardScaler (fit на train)" },
      { label: "Отбор и проверка новых признаков" },
      { label: "Стоимость признака в продакшене" },
      { label: "Чек-лист конструирования признаков" },
    ],
  },
  {
    num: 29,
  ...
  },
  {
    ...
    short: "дерево",
    concepts: [
      { label: "Устройство решающего дерева (узлы и листья)" },
      { label: "Рекурсивное построение дерева" },
      { label: "Выбор вопроса в узле (порог признака)" },
      { label: "Качество разбиения: Gini и MSE" },
      { label: "Энтропия как критерий разбиения" },
      { label: "Прогноз в листе (класс или среднее)" },
      { label: "Вероятность класса и размер листа" },
      { label: "Ступенчатая граница классификации" },
      { label: "Почему граница дерева осевая" },
      { label: "Переобучение глубокого дерева" },
      { label: "Ограничение глубины и post-pruning (ccp_alpha)" },
      { label: "Нужно ли масштабировать признаки для дерева" },
      { label: "Пропуски в DecisionTree" },
      { label: "Кодирование категорий для дерева" },
      { label: "Impurity importance в дереве" },
      { label: "Permutation importance — проверка важности" },
      { label: "Плюсы и ограничения решающего дерева" },
      { label: "Нестабильность дерева при новых данных" },
    ],
  },
  {
    num: 37,
    short: "bagging / RF",
    concepts: [
      { label: "Bootstrap и идея ансамбля" },
      { label: "Сколько уникальных объектов в bootstrap" },
      { label: "Bagging — усреднение прогнозов деревьев" },
      { label: "Разброс ансамбля и связь ошибок моделей" },
      { label: "Out-of-bag оценка без отдельной validation" },
      { label: "Как собирается OOB-прогноз по деревьям" },
      { label: "Random Forest — bagging + случайные признаки" },
      { label: "Лес для классификации и регрессии" },
      { label: "Гиперпараметры random forest (n_estimators, max_depth…)" },
      { label: "Что настраивать в лесу в первую очередь" },
      { label: "Метрики при дисбалансе" },
      { label: "class_weight в Random Forest" },
      { label: "Порог вероятности для бинарной классификации" },
      { label: "Impurity importance в лесу" },
      { label: "Permutation importance на validation" },
      { label: "Random Forest vs Extra Trees" },
      { label: "Плюсы и ограничения bagging и леса" },
    ],
  },
  {
    num: 39,
