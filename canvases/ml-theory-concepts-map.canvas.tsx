...
function assignBigConcept(lessonNum: number, label: string): string {
  ...
  if (
    /Permutation importance|Impurity importance|Feature importance|интерпретац.*коэфф|importance vs|Importance: impurity|Важность признаков/i.test(
      label,
    )
  ) {
    return "importance";
  ...
  if (
    /K-fold|Validation curve|Learning curve|Стратиф|разбиен.*групп|Схема разбиения|fold|Nested cross|Одно разбиение|Переобучение на validation|протокол сравн|Предобработка внутри|независимост.*объект|Разбиение по групп|Вложенная cross|внутренняя validation неверна/i.test(
      label,
    )
  ) {
    return "cv";
  ...
  if (/[Mm]асштаб|StandardScaler|Предобработка данных для дерева/i.test(label)) {
    return "scaling";
  }
  ...
  if (
    /Дисбаланс|class_weight|Взвешивание классов|Порог вероятности|смена порога|Метрики при дисбалансе|class_weight в Random|редком классе/i.test(
      label,
    )
  ) {
    return "imbalance";
...
const LABEL_SPLIT: Record<string, string[]> = {
  ...
  "Качество разбиения: Gini и MSE": [
    ...
    "Information gain — выигрыш от разбиения",
    "MSE как критерий разбиения в регрессии",
  ],
  "Плюсы и ограничения решающего дерева": [
    "Почему одно дерево редко используют в реальных продуктах",
  ...
  "Bagging: bootstrap и усреднение": [
    ...
    "Алгоритм bagging",
    "Одно дерево против bagging",
    "Насколько похожи ошибки деревьев в bagging",
    "Out-of-bag оценка",
  ...
  "Random Forest: разнообразие деревьев": [
    ...
    "Как лес объединяет ответы деревьев",
    "Основные гиперпараметры",
    "Что настраивать первым",
    "Лес при редком классе",
  ],
  "Оценка и эксплуатация ансамблей": [
    "Важность признаков по «загрязнённости» (impurity importance)",
    "Важность по перемешиванию (permutation importance)",
    "OOB, validation и test",
    "Сколько стоит обучить лес",
...
const LABEL_RENAME: Record<string, string> = {
  "Устройство решающего дерева (узлы и листья)":
    "Как устроено решающее дерево",
  "Правила «если — то» и устройство дерева":
    "Как устроено решающее дерево",
  "Выбор вопроса в узле (порог признака)": "Как алгоритм выбирает разбиение",
  "Выбор признака и порога в узле": "Как алгоритм выбирает разбиение",
  "Information gain и качество разбиения":
    "Information gain — выигрыш от разбиения",
  "Рекурсивное построение и остановка": "Рекурсивное построение дерева",
  "Прогноз в листе (класс или среднее)": "Прогноз в листе",
  "Вероятность класса и размер листа": "Прогноз в листе",
  "Прогноз в листе: класс и вероятность": "Прогноз в листе",
  "Ступенчатая граница классификации": "Ступенчатая граница решений",
  "Почему граница дерева осевая": "Ступенчатая граница решений",
  "Ступенчатая и осевая граница решений": "Ступенчатая граница решений",
  "Переобучение глубокого дерева": "Переобучение дерева",
  "Ограничение глубины и post-pruning (ccp_alpha)": "Переобучение дерева",
  "Переобучение, гиперпараметры и обрезка": "Переобучение дерева",
  "Нужно ли масштабировать признаки для дерева":
    "Предобработка данных для дерева",
  "Пропуски в DecisionTree": "Предобработка данных для дерева",
  "Кодирование категорий для дерева": "Предобработка данных для дерева",
  "Масштаб, пропуски и категории в sklearn":
    "Предобработка данных для дерева",
  "Impurity importance в дереве": "Важность признаков",
  "Permutation importance — проверка важности": "Важность признаков",
  "Impurity и permutation importance": "Важность признаков",
  "Нестабильность дерева при новых данных":
    "Почему одно дерево редко используют в реальных продуктах",
  "Почему одно дерево редко в проде":
    "Почему одно дерево редко используют в реальных продуктах",
  "Feature importance в дереве": "Важность признаков",
  ...
  "Лес для классификации и регрессии":
    "Как лес объединяет ответы деревьев",
  "Классификация и регрессия в лесу": "Как лес объединяет ответы деревьев",
  "Что настраивать в лесу в первую очередь": "Что настраивать первым",
  "Метрики при дисбалансе": "Лес при редком классе",
  "class_weight в Random Forest": "Лес при редком классе",
  "Порог вероятности для бинарной классификации": "Лес при редком классе",
  "Дисбаланс: метрики, class_weight и порог": "Лес при редком классе",
  "Impurity importance в лесу":
    "Важность признаков по «загрязнённости» (impurity importance)",
  "Permutation importance на validation":
    "Важность по перемешиванию (permutation importance)",
  "Permutation importance": "Важность по перемешиванию (permutation importance)",
  "Скорость, память и параллельное обучение": "Сколько стоит обучить лес",
  "Random Forest vs Extra Trees": "Extra Trees",
  "Bagging, RF и Extra Trees": "Extra Trees",
  ...
  "Early stopping и post-hoc выбор числа деревьев":
    "Автоматическая остановка обучения (early stopping)",
  "Стратификация по классам в CV": "Схема разбиения повторяет структуру данных",
  "Разбиение по группам, времени и задаче":
    "Схема разбиения повторяет структуру данных",
  "Стратификация, группы и время в CV":
    "Схема разбиения повторяет структуру данных",
  "Типичные утечки внутри cross-validation":
    "Утечки при переборе гиперпараметров",
  "Grid search по сетке гиперпараметров": "Утечки при переборе гиперпараметров",
  "Утечки в CV и grid search": "Утечки при переборе гиперпараметров",
  "Воспроизводимость: seeds и версии артефактов": "Воспроизводимость результата",
...
const ROW_SPLIT_TARGET: Record<string, string> = {
  ...
  "Предобработка данных для дерева": "scaling",
...
const LESSONS: Lesson[] = [
  ...
  },
  {
    ...
    concepts: [
      ...
      { label: "Схема разбиения повторяет структуру данных" },
      { label: "Предобработка внутри каждого fold" },
      { label: "Утечки при переборе гиперпараметров" },
      { label: "Вложенная cross-validation" },
      { label: "Воспроизводимость результата" },
  ...
  },
  {
    ...
    short: "дерево",
    concepts: [
      { label: "Как устроено решающее дерево" },
      { label: "Как алгоритм выбирает разбиение" },
      { label: "Gini impurity — мера смешения классов" },
      { label: "Information gain — выигрыш от разбиения" },
      { label: "Энтропия как критерий разбиения" },
      { label: "Рекурсивное построение дерева" },
      { label: "MSE как критерий разбиения в регрессии" },
      { label: "Прогноз в листе" },
      { label: "Путь одного объекта через дерево" },
      { label: "Ступенчатая граница решений" },
      { label: "Переобучение дерева" },
      { label: "Предобработка данных для дерева" },
      { label: "Важность признаков" },
      { label: "Почему одно дерево редко используют в реальных продуктах" },
  ...
  },
  {
    ...
    concepts: [
      ...
      { label: "Одно дерево против bagging" },
      { label: "Насколько похожи ошибки деревьев в bagging" },
      { label: "Out-of-bag оценка" },
      { label: "Как собирается OOB-прогноз" },
      { label: "Случайный лес" },
      { label: "Случайные признаки и разнообразие деревьев" },
      { label: "Как лес объединяет ответы деревьев" },
      { label: "Основные гиперпараметры" },
      { label: "Что настраивать первым" },
      { label: "Лес при редком классе" },
      { label: "Важность признаков по «загрязнённости» (impurity importance)" },
      { label: "Важность по перемешиванию (permutation importance)" },
      { label: "OOB, validation и test" },
      { label: "Сколько стоит обучить лес" },
  ...
  },
  {
    ...
    concepts: [
      ...
      { label: "Автоматическая остановка обучения (early stopping)" },
